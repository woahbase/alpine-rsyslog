...
# runtime flags
# following are required for listener/forwarder profiles, ignored otherwise
SYS_HOSTNAME    := localhost# $(shell cat /etc/hostname).local
# SYS_HOSTNAME needed so container logs end up in same dir as the docker host device when forwarding.
...
MOUNTFLAGS := \
	...
	-v $(CURDIR)/data/spool:/var/spool/rsyslog \
	# -v /run/systemd/journal:/run/systemd/journal \
	#
	# /run/systemd/journal mount needed for forwarder/listener profiles
PORTFLAGS  := -p 2514:2514 -p 514:514/tcp -p 514:514/udp# --net=host # so other local containers can find it without explicit linking, needs firewall cleared
PUID       := $(shell id -u)
...
OTHERFLAGS := \
	...
	-e PGID=$(PGID) \
	-e PUID=$(PUID) \
	# -e RSYSLOG_PROFILE=default \
	# -e RSYSLOG_CONF=/etc/rsyslog.conf \
	# -e LOGROTATE_CONF=/etc/logrotate.conf \
	# -e LOGROTATE_STATE=/tmp/logrotate.state \
	...
	#
	# --privileged needed for reading kernel logs
